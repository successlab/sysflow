...
static int __init sysflow_test_init(void) {
    int ret;
    ...
    /*test flow lookup*/
    struct sysflow_system_event_hdr *event_hdr_1 = 
                kmalloc(sizeof( struct sysflow_system_event_hdr), GFP_KERNEL);
    ...
    struct sysflow_system_event *event_1 = 
                kmalloc(sizeof(struct sysflow_system_event), GFP_KERNEL);
    event_1->len = sizeof(struct sysflow_system_event);
    event_1->hdr = event_hdr_1;
    event_1->payload = NULL;


    struct sysflow_action *actions_1
                            = kmalloc(sizeof(struct sysflow_action), GFP_KERNEL);
    ...
    struct sysflow_system_event_hdr *event_hdr_2 = 
                kmalloc(sizeof( struct sysflow_system_event_hdr), GFP_KERNEL);
...
static void __exit sysflow_test_exit(void) {
    printk(KERN_INFO "Going to exit test module.\n");
}

module_init(sysflow_test_init);
module_exit(sysflow_test_exit);
