...
/*
* Function to create sysflow flow entry
*/
struct sysflow_entry *create_flow_entry(int pid, struct file_id fid, char *src_name, char *dst_name, int opcode, int mask_val, struct sysflow_action *action){
	...
	memset(mask, 0, sizeof(struct sysflow_mask));
	memset(entry, 0, sizeof(struct sysflow_entry));

	// printk("[S2OS - flow_table.c] creating flow entry. pid: %d, inode_num: %d, src_name: %s, dst_name: %s, opcode: %d\n", pid, fid.inode_num, src_name, dst_name, opcode);

    key.pid = pid;
    key.fid = fid;
	//memcpy(&key.src_name, src_name, SFPFM_MAX_NAME);
	if(key.fid.inode_num == 0){
		memcpy(&key.dst_name, dst_name, SFPFM_MAX_NAME);
	}
    key.opcode = opcode;
    mask->key = key;
    mask->key_mask = mask_val;
    entry->key = key;
    entry->mask = mask;
...
struct sysflow_entry *create_exact_entry(int pid, struct file_id fid, char *src_name, char *dst_name, int opcode){
    struct sysflow_key key;
    struct sysflow_mask *mask = kmalloc(sizeof(struct sysflow_mask), GFP_KERNEL);
...
struct sysflow_entry *create_wildcard_entry(int pid){
    struct sysflow_key key;
    struct sysflow_mask *mask = kmalloc(sizeof(struct sysflow_mask), GFP_KERNEL);
    struct sysflow_action *action = kmalloc(sizeof(struct sysflow_action), GFP_KERNEL);
    struct sysflow_entry *entry = kmalloc(sizeof(struct sysflow_entry), GFP_KERNEL);
...
int test_sysflow_tbl_insert(struct datapath *dp, struct sysflow_entry *entry){
    int ret;
...
int test_sysflow_tbl_remove(struct datapath *dp, struct sysflow_entry *entry){
    int ret;
    struct sysflow_table *table = dp->table;
...
int sysflow_key_extract(struct sysflow_system_event* sse,
	...
	else if (header->src_type == SYSFLOW_SRC_SOCKET){
		memcpy(key->src_name, header->src_name, SFPFM_MAX_NAME);
	}
	else {
	...
	else if (header->dst_type == SYSFLOW_DST_SOCKET){
		memcpy(key->dst_name, header->dst_name, SFPFM_MAX_NAME);
	}
	else{
	...
	if (key->opcode == SYSFLOW_OP_UNKNOWN){
		return -1;
	}
		     ...
		     uint32_t key_mask)
{
	...
	if(_DBG){
		...
		printk(KERN_INFO "hash: %u\n", hash);
		for(i = 0; i<KEY_SRC_SIZE+KEY_DST_SIZE; i++){
			printk("%x ", ((unsigned char *)masked_key)[i]);
		}
		printk("\n");
		printk(KERN_INFO "-------------------------------------------------------------------------\n");
	}
...
static struct mask_array *tbl_mask_array_alloc(int size)
{
	...
	new = kzalloc(sizeof(struct mask_array) +
		      sizeof(struct sysflow_mask *) * size, GFP_NOWAIT);
		       ...
		       const struct sysflow_mask *b)
{
	...
	if(_DBG){
		printk(KERN_INFO "mask1: %d, mask2: %d, isEqual: %s", a->key_mask, b->key_mask, (is_equal)?"true":"false");
	}
...
{
	if(!dst){
		dst = kmalloc(sizeof(struct sysflow_key), GFP_NOWAIT);
	}
...
static struct sysflow_entry *masked_flow_lookup(struct table_instance *ti,  
	...
	struct hlist_node *n;
	uint32_t hash;
	struct sysflow_key *masked_key = kmalloc(sizeof(struct sysflow_key), GFP_ATOMIC);
	...
	// printk("[S2OS - flow_table.c] masked_key. pid: %d, src_name: %s, inode: %d, fid: %d, dst_name: %s, opcode: %d\n", 
	// 	masked_key->pid, masked_key->src_name, masked_key->fid.uuid,
	// 	masked_key->fid.inode_num, masked_key->dst_name, masked_key->opcode);
	// printk("[S2OS - flow_table.c] masked_flow_lookup. head: %p\n", head->first);
	hlist_for_each_entry_rcu(flow_entry, n, head, sysflow_table.node[ti->node_ver]) 
	{ /// list
		// printk("[S2OS - flow_table.c] checking with entry\n");
		// printk("[S2OS - flow_table.c] pid: %d, src_name: %s, inode: %d, fid: %d, dst_name: %s, opcode: %d\n", 
		// 	flow_entry->key.pid, flow_entry->key.src_name, flow_entry->key.fid.uuid,
		// 	flow_entry->key.fid.inode_num, flow_entry->key.dst_name, flow_entry->key.opcode);
		/*	
		...
		*/
		if (flow_entry->sysflow_table.hash == hash)
            // by hongda 
            if (masked_key) { kfree(masked_key); }
			return flow_entry;
	}
...
struct sysflow_entry *sysflow_tbl_lookup(struct sysflow_table *table,
	...
	for (i = 0; i < ma->max; i++)  {
		...
		//printk(KERN_INFO "[S2OS - flow_table.c] In sysflow_tbl_look_up: key mask: %u\n", mask->key_mask);
